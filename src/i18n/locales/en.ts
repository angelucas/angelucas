export default {
  'meta.title': 'Lucas Angelo | .NET Fullstack Developer',
  'meta.description':
    '.NET full stack developer with 7+ years in enterprise applications. APIs, integrations, SQL, and software engineering in Porto Alegre.',

  'loader.label': 'Loading…',

  'nav.skills': 'Skills',
  'nav.projects': 'Projects',
  'nav.experience': 'Experience',
  'nav.contact': 'Contact',

  'hero.greeting': "Hi, I'm",
  'hero.photoAlt': 'Lucas Angelo, full stack developer',
  'hero.name': 'Lucas Angelo',
  'hero.tagline': '.NET · C# · Fullstack · Azure Developer',
  'hero.mono': '// .NET full stack · APIs · integrations · SQL',
  'hero.bio':
    'Software developer with 7+ years building enterprise applications. I work as a full stack .NET developer at Pestana Leilões, with a focus on APIs, integrations, SQL, and legacy systems in agile teams.',
  'hero.cta.projects': 'View projects',
  'hero.cta.contact': 'Get in touch',

  'focus.title': 'Current focus',
  'focus.subtitle': 'AI, automation, and agents to grow as a developer',

  'skills.title': 'Skills',
  'skills.subtitle': 'Stack and practices that matter in enterprise and product teams',
  'skills.backend': 'Backend & .NET',
  'skills.frontend': 'Frontend',
  'skills.data': 'Data',
  'skills.cloud': 'Cloud & DevOps',
  'skills.practices': 'Quality & delivery',

  'projects.title': 'Projects',
  'projects.subtitle': 'Selected work and experiments',
  'projects.viewCode': 'View code',
  'projects.viewDemo': 'View demo',

  'experience.title': 'Experience',
  'experience.subtitle': 'Seven+ years building and evolving enterprise software',

  'contact.title': 'Contact',
  'contact.subtitle': "Let's talk?",
  'contact.message': 'Open to opportunities, collaborations, and networking.',
  'contact.email': 'Email',
  'contact.github': 'GitHub',
  'contact.linkedin': 'LinkedIn',

  'footer.rights': 'All rights reserved.',
} as const;
